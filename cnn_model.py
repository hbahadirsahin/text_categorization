    ...
    def __init__(self, sentence_length, num_classes, vocabulary_size, embedding_size, filter_sizes, num_filters,
        ...
        with tf.device('/gpu:0'):
            ...
            with tf.name_scope("fully_connected_layer1"):
                ...
                fc1_out = tf.nn.relu(fc1_bn, name="fc1_out")
            ...
            with tf.name_scope("accuracytopk"):
                self.pred_top_2 = tf.reduce_mean(
                    tf.cast(tf.nn.in_top_k(self.predictions, tf.argmax(self.input_y, 1), k=2), tf.float32), name="top2")
                self.pred_top_3 = tf.reduce_mean(
                    tf.cast(tf.nn.in_top_k(self.predictions, tf.argmax(self.input_y, 1), k=3), tf.float32), name="top3")
                self.pred_top_4 = tf.reduce_mean(
                    tf.cast(tf.nn.in_top_k(self.predictions, tf.argmax(self.input_y, 1), k=4), tf.float32), name="top4")
                self.pred_top_5 = tf.reduce_mean(
                    tf.cast(tf.nn.in_top_k(self.predictions, tf.argmax(self.input_y, 1), k=5), tf.float32), name="top5")
